...
#include "raphics/modifiers/caps_word_enabled.qgf.h"
#include "raphics/modifiers/caps_word_shift_pressed.qgf.h"
#include "raphics/modifiers/command_pressed.qgf.h"
#include "raphics/modifiers/command_unpressed.qgf.h"
#include "raphics/modifiers/control_pressed.qgf.h"
#include "raphics/modifiers/control_unpressed.qgf.h"
#include "raphics/modifiers/option_pressed.qgf.h"
#include "raphics/modifiers/option_unpressed.qgf.h"
#include "raphics/modifiers/shift_pressed.qgf.h"
#include "raphics/modifiers/shift_unpressed.qgf.h"

#include "raphics/layers/base_layer_icon_colored.qgf.h"
#include "raphics/layers/base_layer_text_colored.qgf.h"
#include "raphics/layers/lower_layer_icon_colored.qgf.h"
#include "raphics/layers/lower_layer_text_colored.qgf.h"
#include "raphics/layers/nav_3d_layer_icon_colored.qgf.h"
#include "raphics/layers/nav_3d_layer_text_colored.qgf.h"
#include "raphics/layers/raise_layer_icon_colored.qgf.h"
#include "raphics/layers/raise_layer_text_colored.qgf.h"
...
static painter_image_handle_t shift;
static painter_image_handle_t control;
static painter_image_handle_t option;
static painter_image_handle_t command;
static painter_image_handle_t caps_word;
...
bool module_post_init_user(void) {
    ...
    layer_mapping[_COUNT].icon             = qp_load_image_mem(gfx_undefined_layer_icon);
    layer_mapping[_COUNT].text             = qp_load_image_mem(gfx_undefined_layer_text);
    ...
    caps_word.pressed   = qp_load_image_mem(gfx_caps_word_enabled);
    caps_word.unpressed = qp_load_image_mem(gfx_caps_word_shift_pressed);
...
bool display_module_housekeeping_task_user(bool second_display) {
    ...
    if (!second_display) {
        ...
        if (!layer_initialized || current_layer != last_layer) {
            ...
            qp_drawimage(lcd_surface, (LCD_WIDTH - layer->icon->width) >> 1, ((LCD_HEIGHT >> 1) - layer->icon->height) >> 1, layer_mapping[layer_idx]);
            qp_drawimage(lcd_surface, (LCD_WIDTH - layer->text->width) >> 1, (LCD_HEIGHT - layer->icon->height) >> 1, layer_mapping[layer_idx]);
        ...
        if (!modifiers_initialized || in_or_out_of_no_hrm || current_modifiers != last_modifiers || caps_word_changed) {
            ...
            if (changed_modifiers & MOD_MASK_CTRL || in_or_out_of_no_hrm) {
                ...
                if (current_layer == _BASE_NO_HRM) {
                    qp_rect(lcd, x, y, width - 1, height - 1, HSV_BLACK, true);
                } else {
            ...
            if (changed_modifiers & MOD_MASK_SHIFT || caps_word_changed) {
                ...
                painter_image_handle_t   status   = current_modifiers & MOD_MASK_GUI ? modifier.pressed : modifier.unpressed;
                const uint16_t           width    = status->width;
                const uint16_t           height   = status->height;
                const uint16_t           x        = (LCD_WIDTH - width) >> 1;
                const uint16_t           y        = LCD_HEIGHT - (((LCD_HEIGHT >> 1) + height) >> 1);
